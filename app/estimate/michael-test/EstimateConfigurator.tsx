...
export default function EstimateConfigurator() {
  ...
  const monthly = Math.round((total * 1.18) / 60);

  const activePresetId = useMemo(
    () => PRESETS.find((p) => sameZoneSet(p.zoneIds, selectedZones))?.id ?? null,
    [selectedZones]
  );
  ...
  return (
    <div className="pb-32 sm:pb-28">
      {/* PAGE HEADER — letterhead for this personalized estimate */}
      <section className="bg-[#1a2e44] text-white">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6">
          <Image
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-12">
        ...
        {/* ZONES */}
        <section>
          <h2 className="text-2xl sm:text-3xl font-bold text-[#1a2e44] mb-1">Zones</h2>
        ...
        <p className="text-sm text-gray-500 text-center">
          Questions about this estimate? Call or text{" "}
          <a href={`tel:${PHONE_TEL}`} className="text-[#3A6A96] font-semibold hover:underline">
            {PHONE_DISPLAY}
          </a>{" "}
          or email{" "}
          <a href={`mailto:${EMAIL}`} className="text-[#3A6A96] font-semibold hover:underline">
            {EMAIL}
          </a>
          .
        </p>
      ...
      <div className="fixed bottom-0 inset-x-0 z-40 bg-white border-t border-gray-200 shadow-[0_-4px_16px_rgba(0,0,0,0.08)]">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-3 sm:py-4 flex items-center justify-between gap-3 flex-wrap">
          ...
          <div className="text-right">
            ...
            <div className="text-xs sm:text-sm text-gray-500">
              or est. {currency(monthly)}/mo with financing{" "}
              <span className="text-gray-400">— estimated, subject to lender approval</span>
            </div>
          </div>
        </div>
      </div>
