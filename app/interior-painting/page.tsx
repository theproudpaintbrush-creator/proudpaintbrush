...
export default function InteriorPaintingHub() {
  ...
  const reviews = getReviewsForService("interior");
  const schemas = [...buildHubSchemas("interior", hub.title, hub.metaDescription, hub.faqs, subServices), ...buildReviewSchema(reviews)];
  ...
  return (
    <>
      ...
      <ServiceHub hub={hub} subServices={subServices} cities={cities} relatedPosts={relatedPosts} reviews={reviews} />
    </>
  );
}
